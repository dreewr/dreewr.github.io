...
(() => {
  ...
  async function carregar() {
    ...
    let resp;
    try {
      resp = await LeiloesConfig.fetchAPI(`/api/imoveis?${params}`);
    } catch (e) {
    ...
    tbody.innerHTML = data.items.map(item => `
      <tr>
        <td><a href="/analise?id=${encodeURIComponent(item.id)}">${item.apelido || item.id}</a></td>
        <td>${item.endereco || '—'}</td>
  ...
  // Exportar CSV — abre URL do backend
  document.getElementById('btn-exportar').addEventListener('click', () => {
    window.open(LeiloesConfig.url('/api/exportar-csv'), '_blank');
  });

  // Configurar backend
  const dlg = document.getElementById('dlg-config');
  document.getElementById('btn-configurar').addEventListener('click', () => {
    document.getElementById('cfg-url').value = LeiloesConfig.get();
    document.getElementById('cfg-current').textContent = 'Atual: ' + (LeiloesConfig.get() || '(mesma origem)');
    dlg.showModal();
  });
  document.getElementById('cfg-cancelar').addEventListener('click', () => dlg.close());
  document.getElementById('cfg-salvar').addEventListener('click', () => {
    LeiloesConfig.set(document.getElementById('cfg-url').value.trim());
    dlg.close();
    carregar();
  });
