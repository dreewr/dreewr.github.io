...
(() => {
  ...
  async function carregarSeId() {
    ...
    const resp = await LeiloesConfig.fetchAPI(`/api/imoveis/${encodeURIComponent(id)}`);
  ...
  function preencherForms() {
    ...
    if (!i) return;
    $('apelido').value = i.apelido || '';
    $('texto-edital').value = i.edital?.texto || '';
    $('texto-matricula').value = i.matricula?.texto || '';
  ...
  $('btn-analisar').addEventListener('click', async () => {
    const edital = $('texto-edital').value.trim();
    const matricula = $('texto-matricula').value.trim();
    if (!edital && !matricula) {
      $('erro-analise').textContent = 'Cole o edital e/ou a matrícula.';
      $('erro-analise').hidden = false;
      return;
    }
    ...
    try {
      const resp = await LeiloesConfig.fetchAPI('/api/analisar', {
        method: 'POST',
  ...
  $('btn-calcular').addEventListener('click', async () => {
    ...
    const resp = await LeiloesConfig.fetchAPI('/api/calcular', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function salvarImovel() {
    if (!state.imovel) return;
    const resp = await LeiloesConfig.fetchAPI('/api/imoveis', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // Configurar backend
  const dlg = document.getElementById('dlg-config');
  document.getElementById('btn-configurar').addEventListener('click', () => {
    $('cfg-url').value = LeiloesConfig.get();
    $('cfg-current').textContent = 'Atual: ' + (LeiloesConfig.get() || '(mesma origem)');
    dlg.showModal();
  });
  $('cfg-cancelar').addEventListener('click', () => dlg.close());
  $('cfg-salvar').addEventListener('click', () => {
    LeiloesConfig.set($('cfg-url').value.trim());
    dlg.close();
  });
